...
onMounted(() => {
  ...
  iframeSrc.value = `http://${ip}:81/index.html#/statistic_analysis?ip=${ip}&port=81`;
});
...
watch(selectedStation, (newVal) => {
  iframeSrc.value = `http://${newVal.ip}:81/index.html#/statistic_analysis?ip=${newVal.ip}&port=81`;
});
